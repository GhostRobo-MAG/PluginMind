...
class Settings:
    ...
    def __init__(self):
        ...
        self.http_timeout_seconds = self._parse_float("HTTP_TIMEOUT_SECONDS", "250")
        ...
        self.backend_session_secret = os.getenv("BACKEND_SESSION_SECRET")
        # Optional cookie domain for cross-subdomain deployments (e.g., .example.com)
        ...
        if self.testing:
            ...
            if not self.backend_session_secret:
                self.backend_session_secret = "test-backend-session-secret-32-chars-min"
    ...
    def _validate_configuration(self) -> None:
        ...
        # Validate backend session secret
        if not self.backend_session_secret or len(self.backend_session_secret) < 32:
            errors.append("BACKEND_SESSION_SECRET is missing or too short (minimum 32 characters required)")
